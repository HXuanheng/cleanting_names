...
def clean_person(word):
    ...
    for name in re.split('[&/]', word):
                ...
                if title == "":
                    for tt in ['dr', 'mr', 'ms', 'mrs', 'prof']:
                        if tt in suffix:
                            title = tt
                            suffix = suffix.replace(tt,'')
                            suffix = suffix.strip()
                ...
                for m in mid.split():
                    ...
                    std_mid.append(m)
                mid = ' '.join(std_mid)
    return first, mid, last, title, suffix, roman

# name1 = 'MURPHY, CHRISTOPHER Jr. MR. III'
# print(clean_person(name1))

# name2 = 'michele fumagalli jr'
# print(clean_person(name2))